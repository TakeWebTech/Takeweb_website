...
const benefits = [
  ...
  },
  {
    icon: Users,
    title: "Team Events",
    description: "Regular team building and offsite retreats",
  },
];
...
const culture = [
  {
    icon: Zap,
    title: "Move Fast",
    description: "We ship quickly and iterate often.",
  },
  {
    icon: Users,
    title: "Collaborate Openly",
    description: "We work transparently and support each other.",
  },
  {
    icon: Heart,
    title: "Care Deeply",
    description: "We care about our work and each other's wellbeing.",
  },
];
...
export default function CareersPage() {
  ...
  const [selectedJob, setSelectedJob] = useState<ErpJob | null>(null);
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const res = await fetch(careersApi());
        if (res.ok) {
  ...
  const filteredPositions = useMemo(() => {
    return jobs.filter((job) => {
      ...
      const matchesType =
        selectedType === "All" || job.employment_type === selectedType;
        ...
        <div className="container-main relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-lg text-[var(--text-tertiary)] mb-8">
              Join a team of passionate technologists working on challenging
              problems for some of the world&apos;s leading enterprises.
            </p>
            <Link
              href="#positions"
        ...
        <div className="container-main">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((benefit, index) => (
              <div
                key={index}
                className="flex gap-4 p-6 bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-2xl hover:border-[var(--border-secondary)] transition-colors"
              >
              ...
              >
                <Card3D className="group h-full">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    ...
                    >
                      {position.employment_type || "Open role"}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-[var(--border-primary)] text-sm text-[var(--text-muted)]">
                    ...
                    {position.salary !== null && position.salary !== "" && (
                      <span className="flex items-center gap-1">
                        {position.salary}
                      </span>
                    )}
          ...
          >
            ...
            <div className="p-8">
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-[var(--text-tertiary)]">
                  ...
                  {selectedJob.posted_on && (
                    <span>Posted {formatJobDate(selectedJob.posted_on)}</span>
                  )}
                  {selectedJob.closes_on && (
                    <span>Closes {formatJobDate(selectedJob.closes_on)}</span>
                  )}
                </div>
              </div>

              {/* Salary */}
              {selectedJob.salary !== null && selectedJob.salary !== "" && (
              ...
              <div className="border-t border-[var(--border-primary)] pt-6">
                ...
                <div className="mb-4">
                  <Link
                    href={`/careers/${encodeURIComponent(selectedJob.id)}`}
                    className="mr-3 inline-flex items-center justify-center px-6 py-3 font-semibold text-[var(--text-primary)] border border-[var(--border-secondary)] rounded-xl hover:border-amber-500 transition-all"
                  >
                    View Details
                  </Link>
                  <Link
                    href={`/careers/apply?job=${encodeURIComponent(selectedJob.id)}`}
