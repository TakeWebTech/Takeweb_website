...
export class SeoService {
        ...
        if (!settings) {
            ...
            settings = await this.prisma.seoSettings.create({
                data: {
                    ...
                    titleSeparator: '|',
                    metaDescription: 'TakeWeb provides enterprise digital solutions including web development, app development, and AI solutions.',
    ...
    async updateSeoMeta(entityType: string, entityId: string, dto: UpdateSeoMetaDto) {
        const existing = await this.prisma.seoMeta.findUnique({
            where: {
                entityType_entityId: { entityType, entityId },
            },
    ...
    async analyzeContent(dto: AnalyzeContentDto): Promise<SeoScoreResult> {
    ...
    async analyzeUrl(url: string): Promise<SeoScoreResult> {
        ...
        throw new BadRequestException('URL analysis requires fetching external content');
    }
    ...
    async createRedirect(dto: CreateRedirectDto) {
        ...
        if (existing) {
            throw new ConflictException('A redirect already exists for this source URL');
        }

        // Check for redirect loops
        if (dto.sourceUrl === dto.targetUrl) {
            throw new BadRequestException('Source and target URLs cannot be the same');
        }
    ...
    async createBulkKeywords(dto: BulkAddKeywordsDto) {
        ...
        for (const kw of dto.keywords) {
            try {
                ...
                results.failed++;
                results.errors.push(`${kw.keyword}: ${error.message}`);
            }
        }
    ...
    async updateKeywordRanking(id: string, position: number, searchengine = 'google') {
        const keyword = await this.getKeyword(id);
        const history = (keyword.positionHistory as any[]) || [];
        ...
        return this.prisma.keywordTracking.update({
            ...
            data: {
                ...
                previousPosition,
                bestPosition: Math.min(keyword.bestPosition || 100, position),
                positionHistory: filteredHistory,
    ...
    async runSeoAudit(dto: RunSeoAuditDto): Promise<SeoAuditResult> {
        ...
        const [posts, services, projects] = await Promise.all([
            this.prisma.blogPost.findMany({ where: { published: true } }),
            this.prisma.service.findMany({ where: { active: true } }),
            this.prisma.project.findMany({ where: { featured: true } }),
        ]);

        // Audit each category
        for (const categoryKey of Object.keys(SEO_AUDIT_CHECKS)) {
            const categoryChecks = SEO_AUDIT_CHECKS[categoryKey as keyof typeof SEO_AUDIT_CHECKS];
            const issues: SeoAuditIssue[] = [];

            for (const check of categoryChecks) {
                const checkResult = await this.runAuditCheck(check.id, posts, services, projects);
                if (checkResult) {
        ...
        const result: SeoAuditResult = {
            ...
            totalIssues: categories.reduce((sum, cat) => sum + cat.failed, 0),
            ...
            case 'missing_meta_titles':
                ...
                if (missingTitles > 0) {
                    return {
                        ...
                        affectedUrls: posts.filter(p => !p.metaTitle).map(p => `/blog/${p.slug}`),
            ...
            case 'missing_meta_descriptions':
                ...
                if (missingDescs > 0) {
                    return {
                        ...
                        affectedUrls: posts.filter(p => !p.metaDescription).map(p => `/blog/${p.slug}`),
            ...
            case 'missing_og_images':
                ...
                if (missingImages > 0) {
                    return {
                        ...
                        affectedUrls: posts.filter(p => !p.ogImage).map(p => `/blog/${p.slug}`),
    ...
    async generateLinkSuggestions(entityType: string, entityId: string) {
        ...
        if (entityType === 'BlogPost') {
            const post = await this.prisma.blogPost.findUnique({ where: { id: entityId } });
            if (post) {
        ...
        for (const suggestion of suggestions.slice(0, 10)) {
            await this.prisma.linkSuggestion.upsert({
                where: {
                    sourceEntityType_sourceEntityId_targetEntityType_targetEntityId: {
    ...
    async getDashboardStats() {
        const [
            ...
            this.prisma.error404Log.count({ where: { resolved: false } }),
            this.prisma.keywordTracking.count(),
